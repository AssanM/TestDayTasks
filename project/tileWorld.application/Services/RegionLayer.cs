...
public class RegionLayer : IRegionLayer
{
        ...
        await Task.Run(() =>
        {
            ...
            for (int y = 0; y < mapHeight; y += regionHeight)
            {
                for (int x = 0; x < mapWidth; x += regionWidth)
                {
                    ...
                    _regions[id] = new Region(id, $"Регион #{id}");
    ...
    public Task<ushort> GetRegionIdAtAsync(int x, int y) =>
        Task.FromResult(_regionMap[x, y]);

    public Task<Region> GetRegionByIdAsync(ushort id) =>
        Task.FromResult(_regions[id]);

    public Task<bool> TileBelongsToRegionAsync(int x, int y, ushort regionId) =>
        Task.FromResult(_regionMap[x, y] == regionId);
